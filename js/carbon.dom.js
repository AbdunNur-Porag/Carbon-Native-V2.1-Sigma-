function buildAPI(el) {
  const api = {
    ...
    id(ids = []) {
      if (ids.length > 1) {
        throw new Error("Only one ID is allowed per element.");
      }
      el.id = ids[0] || "";
      return api;
    },
      ...
      const parse = (target, container) => {
        Object.entries(container).forEach(([key, val]) => {
          ...
          if (val && typeof val === "object" && val._el) {
            ...
            target._children[key].forEach(e =>
              target._el.appendChild(e._el)
            );
            target[key] = val[0];
